...
namespace MagicOClockGenerator
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            Label lbl = new Label();
            lbl.Text = "Bộ tạo giao diện (GitHub URL)";
        ...
        private void BtnGenerate_Click(object sender, EventArgs e)
        {
            try
            {
                if (!Directory.Exists(rootPath)) { MessageBox.Show("Không thấy: " + rootPath); return; }
                ...
                foreach (var dirPath in subDirs)
                {
                    ...
                    string jpgPath = Path.Combine(rootPath, dirName + ".jpg");
                    string txtPath = Path.Combine(rootPath, dirName + ".txt");
                    if (File.Exists(jpgPath) && File.Exists(txtPath))
                    {
                        string title = File.ReadAllLines(txtPath).FirstOrDefault() ?? dirName;
                ...
                MessageBox.Show("Xong! Đã cập nhật link GitHub cho ảnh và download.");
            }
        ...
        private void GenerateSimpleJson(List<string> htmlFiles)
        {
            ...
            sb.AppendLine("{\n  \"data\": [");
            var entries = htmlFiles.Select(f => $"    \"{f}\"").ToList();
            sb.Append(string.Join("," + Environment.NewLine, entries));
            sb.AppendLine("\n  ]\n}");
        ...
        private void GenerateMainListFile(List<CategoryInfo> categories)
        {
            ...
            {
                string githubImg = GITHUB_URL + cat.Image;
                sb.Append($@"
            <a href='{cat.SafeFileName}' class='category_item'>
                <img src='{githubImg}' alt='{cat.Title}'>
        ...
        private void GenerateDetailFile(CategoryInfo cat)
        {
            ...
            foreach (var item in cat.Items)
            {
                ...
                <div class='view_item'>
                    <div class='vi_left'>
                        <img src='{githubImg}' alt='{item.BmpFile}'>
                        <div class='file-name'>{item.ClkFile}</div>
                    </div>
                    <button class='btn-download' data-url='{githubClk}'>Download</button>
                </div>");
            }
        ...
        .view_item { background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; padding: 10px; display: inline-flex; flex-direction: column; align-items: center; width: 100px; }
        .vi_left img { width: 80px; height: 80px; border-radius: 8px; object-fit: cover; }
        .btn-download { background: linear-gradient(135deg, #4facfe, #00f2fe); color: white; border: none; border-radius: 6px; padding: 6px 0; width: 100%; font-size: 11px; font-weight: bold; margin-top: 8px; cursor: pointer; }";
...
.overlay {{ display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.6); z-index: 9999; justify-content: center; align-items: center; backdrop-filter: blur(5px); }}
.loader {{ border: 3px solid rgba(255,255,255,0.1); border-top: 3px solid #4facfe; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; margin-bottom: 10px; }}
@keyframes spin {{ 0% {{ transform: rotate(0deg); }} 100% {{ transform: rotate(360deg); }} }} {s}</style></head>
<body><div id='overlay' class='overlay'><div style='text-align:center; color:white;'><div class='loader' style='margin:0 auto 10px;'></div><p id='status-text'>Processing...</p></div></div>
...
websocket.onmessage = function(evt) {{ if (evt.data === 'SAVEOK') {{ document.getElementById('status-text').innerText = 'OK!'; setTimeout(() => {{ document.getElementById('overlay').style.display='none'; }}, 1000); }} }};
}}
function download(url) {{ document.getElementById('overlay').style.display='flex'; if (websocket && websocket.readyState === 1) websocket.send('#DLD@' + url); }}
document.getElementById('items-container').addEventListener('click', function(e) {{
var t = e.target.closest('.btn-download'); if (t) download(t.getAttribute('data-url'));
